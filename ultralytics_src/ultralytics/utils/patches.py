...
def imread(filename: str, flags: int = cv2.IMREAD_COLOR) -> np.ndarray | None:
    ...
    if filename.endswith((".tiff", ".tif")):
        # Use PIL for TIFF to avoid OpenCV libtiff geospatial tag warnings
        try:
            from PIL import Image
            img_pil = Image.open(filename)
            # PIL returns RGB; convert to BGR for OpenCV/Ultralytics pipeline
        ...
        if success:
            im = frames[0] if len(frames) == 1 and frames[0].ndim == 3 else np.stack(frames, axis=2)
