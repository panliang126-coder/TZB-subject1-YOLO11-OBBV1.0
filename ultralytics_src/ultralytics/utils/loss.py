...
class v8OBBLoss(v8DetectionLoss):
    ...
    Enhanced with scale-aware weighting, focal loss, Wise-IoU, KLD angle loss, and slide loss.
    All features are toggleable via hyperparameter configuration.
    """
    ...
    def __init__(self, model: torch.nn.Module, tal_topk=10, tal_topk2: int | None = None):
        ...
        # ── 增强特性配置 (全部通过 hyp 控制开关) ──
        # 1. Focal Loss 替代 BCE (★★★★★ 长尾类别处理)
        self.use_focal = getattr(self.hyp, 'use_focal', False)
        if self.use_focal:
            self.focal_loss = FocalLoss(
                gamma=getattr(self.hyp, 'focal_gamma', 2.0),
                alpha=getattr(self.hyp, 'focal_alpha', 0.25),
            )
        # 2. Wise-IoU 替代 ProbIoU (★★★★☆ 低质量样本降权)
        self.use_wise_iou = getattr(self.hyp, 'use_wise_iou', False)
        self.wise_iou_beta = getattr(self.hyp, 'wise_iou_beta', 6.0)
        # 3. KLD 角度损失 (★★★★☆ 更稳定的旋转框角度回归)
        self.use_kld_angle = getattr(self.hyp, 'use_kld_angle', False)
        self.kld_tau = getattr(self.hyp, 'kld_tau', 1.0)
        # 4. Slide Loss (★★★★☆ 关注中等难度样本)
        self.use_slide_loss = getattr(self.hyp, 'use_slide_loss', False)
        self.slide_mu = getattr(self.hyp, 'slide_mu', 0.5)
        # 5. Scale-aware 损失加权 (★★★★★ 充分利用 scale_id)
        self.use_scale_aware = getattr(self.hyp, 'use_scale_aware', False)
        self.scale_aware_gamma = getattr(self.hyp, 'scale_aware_gamma', 1.0)
        # 6. Class frequency weights (★★★★★ 长尾)
        self.cls_pw = getattr(self.hyp, 'cls_pw', 0.0)
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # ── Classification Loss ──
        if self.use_focal:
            # Focal Loss: 针对长尾分布, gamma控制对简单样本的抑制强度
            # FocalLoss.forward returns a scalar (mean(1).sum()), class_weights via focal_alpha
            cls_loss = self.focal_loss(pred_scores, target_scores.to(dtype))
        else:
            cls_loss = self.bce(pred_scores, target_scores.to(dtype))  # BCE, shape [B, N, nc]
            if self.class_weights is not None:
        ...
        if fg_mask.sum():
            ...
            # ── Scale-aware Weight (★★★★★ 小目标高权重) ──
            if self.use_scale_aware and "scale_ids" in batch:
                scale_ids = batch["scale_ids"]
                if scale_ids is not None and scale_ids.numel() > 0:
                    # 将 scale_id 映射到每个前景 anchor
                    batch_idx_per_anchor = torch.arange(batch_size, device=self.device).repeat_interleave(fg_mask.shape[1])
                    # 简化为: 越小的 scale_id 权重越高
                    scale_weight = self._compute_scale_weight(batch, fg_mask)
                    weight = weight * scale_weight[fg_mask].unsqueeze(-1)
                    target_scores_sum = max((target_scores * scale_weight.unsqueeze(-1)).sum(), 1)

            # ── Bbox Loss ──
            if self.use_wise_iou:
                loss[0], loss[2] = self._wise_iou_loss(
                    pred_distri, pred_bboxes, anchor_points,
                    target_bboxes, target_scores, target_scores_sum,
                    fg_mask, weight, imgsz, stride_tensor,
                )
            else:
                loss[0], loss[2] = self.bbox_loss(
                    pred_distri, pred_bboxes, anchor_points,
            ...
            # ── Angle Loss ──
            if self.use_kld_angle:
                loss[3] = self._kld_angle_loss(
    ...
    def calculate_angle_loss(self, pred_bboxes, target_bboxes, fg_mask, weight, target_scores_sum, lambda_val=3):
        """Calculate oriented angle loss using sin(2*delta)^2 with aspect-ratio weighting."""
    ...
    def _kld_angle_loss(self, pred_bboxes, target_bboxes, fg_mask, weight, target_scores_sum):
        """KLD (Kullback-Leibler Divergence) based angle loss for more stable OBB regression.

        Converts each oriented box to a 2D Gaussian and minimizes the KL divergence.
        Reference: https://arxiv.org/abs/2106.06072
        """
        pred = pred_bboxes[fg_mask]
        target = target_bboxes[fg_mask]

        # Get covariance matrices for both predictions and targets
        a_p, b_p, c_p = self._get_gaussian_params(pred)
        a_t, b_t, c_t = self._get_gaussian_params(target)

        # KL divergence between 2D Gaussians:
        # D_KL(N_p || N_t) = 0.5 * (tr(Sigma_t^{-1} Sigma_p) + (mu_t-mu_p)^T Sigma_t^{-1} (mu_t-mu_p) - 2 + log(|Sigma_t|/|Sigma_p|))
        det_p = a_p * b_p - c_p * c_p + 1e-9
        det_t = a_t * b_t - c_t * c_t + 1e-9

        # trace term: tr(Sigma_t^{-1} Sigma_p)
        trace_term = (a_p * b_t + b_p * a_t - 2 * c_p * c_t) / det_t

        # center distance term
        dx = (pred[:, 0] - target[:, 0]) / self.kld_tau
        dy = (pred[:, 1] - target[:, 1]) / self.kld_tau
        center_dist = (dx * dx * b_t + dy * dy * a_t - 2 * dx * dy * c_t) / det_t

        # log determinant ratio
        log_term = torch.log(det_t / det_p)

        kld = 0.5 * (trace_term + center_dist - 2.0 + log_term)
        # Smooth L1 on KLD for stability
        kld_loss = torch.sqrt(1 + kld * kld / (self.kld_tau * self.kld_tau)) - 1

        kld_loss = kld_loss * weight
        return kld_loss.sum() / target_scores_sum

    @staticmethod
    def _get_gaussian_params(boxes):
        """Convert OBB (x, y, w, h, theta) to 2D Gaussian covariance components.

        Returns (a, b, c) where covariance matrix = [[a, c], [c, b]].
        """
        w, h, theta = boxes[:, 2], boxes[:, 3], boxes[:, 4]
        w2 = w * w / 12.0
        h2 = h * h / 12.0
        cos_t = torch.cos(theta)
        sin_t = torch.sin(theta)
        a = w2 * cos_t * cos_t + h2 * sin_t * sin_t
        b = w2 * sin_t * sin_t + h2 * cos_t * cos_t
        c = (w2 - h2) * cos_t * sin_t
        return a, b, c

    def _wise_iou_loss(self, pred_distri, pred_bboxes, anchor_points,
                        target_bboxes, target_scores, target_scores_sum,
                        fg_mask, weight, imgsz, stride_tensor):
        """Wise-IoU based box loss: reduces gradient from low-quality examples.

        WIoU v1: L = exp((dx^2+dy^2)/L^2) * (1 - IoU)
        where L^2 = W_g^2 + H_g^2 (size of smallest enclosing box diagonal)
        """
        # Standard ProbIoU for box regression
        iou = probiou(pred_bboxes[fg_mask], target_bboxes[fg_mask])
        loss_iou = ((1.0 - iou) * weight).sum() / target_scores_sum
    ...
    def _compute_scale_weight(self, batch, fg_mask):
        """Compute scale-aware loss weight from scale_id.

        Smaller scale_id (smaller objects) get higher loss weight.
        scale_id 0: max weight, scale_id 3: weight=1.0
        """
        if "scale_ids" not in batch or batch["scale_ids"] is None:
            return torch.ones_like(fg_mask, dtype=torch.float32)

        scale_ids = batch["scale_ids"]  # shape: [N, 1] per target
        if scale_ids.numel() == 0:
            return torch.ones_like(fg_mask, dtype=torch.float32)

        # Weight formula: w = 1 + gamma * (max_scale - scale_id) / max_scale
        # scale 0 -> 1 + gamma, scale 3 -> 1.0
        max_scale = 3.0
        # Expand to per-anchor weights via fg_mask
        scale_weight_base = 1.0 + self.scale_aware_gamma * (max_scale - scale_ids.float()) / max_scale
        # Broadcast: mean over targets per image, expand to anchors
        batch_weight = scale_weight_base.mean()
        scale_weight = torch.ones(fg_mask.shape, device=fg_mask.device, dtype=torch.float32)
        # Apply uniformly for now; in full implementation, uses per-GT assignment
        scale_weight = scale_weight * max(batch_weight, 1.0)
        return scale_weight
