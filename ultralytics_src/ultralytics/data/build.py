...
def build_dataloader(
    ...
    Args:
        ...
        persistent (bool, optional): Whether to keep workers alive across epochs. 强烈建议开启以消除 worker 重建开销.
    ...
    # 优化: 允许用户通过 workers 参数精确控制, 不再自动封顶到 cpu_count/gpu_count
    # 原逻辑: nw = min(os.cpu_count() // max(nd, 1), workers)
    # 新逻辑: 使用 min(2 * os.cpu_count() // max(nd, 1), workers) 给予更多空间
    ...
    return InfiniteDataLoader(
        ...
        persistent_workers=persistent and nw > 0,  # ☆ 消除 epoch 间 worker 重建开销
