...
class BaseTrainer:
    ...
    def _do_train(self):
        ...
        # ── Profiling 初始化 ──
        # 通过 args.profile_loader 开关控制, 默认开启以帮助诊断性能瓶颈
        self._profiler = None
        if getattr(self.args, "profile_loader", False):
            from ultralytics.utils.profile_loader import DataLoadProfiler
            self._profiler = DataLoadProfiler(enabled=True, save_dir=self.save_dir)
            LOGGER.info("📊 DataLoader Profiling ENABLED")
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                # ── Profiling: 记录 batch 间 idle 和 preprocess ──
                if profiler:
                ...
                try:
                    with autocast(self.amp):
                        # Preprocess (CPU->GPU transfer + normalize)
                        if profiler:
                            t_pre = time.perf_counter()
                        batch = self.preprocess_batch(batch)
                        if profiler:
                            profiler.record_stage("collate", (time.perf_counter() - t_pre) * 1000)

                        # Forward + Loss (GPU timing with CUDA events)
                        fwd_start_ev = torch.cuda.Event(enable_timing=True) if profiler else None
                        fwd_end_ev = torch.cuda.Event(enable_timing=True) if profiler else None
                        if profiler:
                            fwd_start_ev.record()
                        ...
                        if profiler:
                            fwd_end_ev.record()
                            profiler.record_gpu_stage("forward", fwd_start_ev, fwd_end_ev)
                    ...
                    # Backward (GPU timing)
                    bwd_start_ev = torch.cuda.Event(enable_timing=True) if profiler else None
                    bwd_end_ev = torch.cuda.Event(enable_timing=True) if profiler else None
                    if profiler:
                        bwd_start_ev.record()
                    self.scaler.scale(self.loss).backward()
                    if profiler:
                        bwd_end_ev.record()
                        profiler.record_gpu_stage("backward", bwd_start_ev, bwd_end_ev)
                ...
                if ni - last_opt_step >= self.accumulate:
                    # ── Profiling: GPU timing for optimizer step ──
                    opt_start_ev = torch.cuda.Event(enable_timing=True) if profiler else None
                    opt_end_ev = torch.cuda.Event(enable_timing=True) if profiler else None
                    if profiler:
                        opt_start_ev.record()
                    self.optimizer_step()
                    if profiler:
                        opt_end_ev.record()
                        profiler.record_gpu_stage("optimizer", opt_start_ev, opt_end_ev)
                    last_opt_step = ni
            ...
            # ── Profiling: epoch 结束, 输出报告 ──
            if profiler:
                profiler.end_epoch(epoch)
        ...
        # ── Profiling: 最终报告 ──
        if self._profiler:
            self._profiler.save_final_report()
        self._clear_memory()
        unset_deterministic()
